...
{
    int16_t *p_dest = (int16_t *) &psensor_ssss_data_block_prev->p_data;
    int dataElementSize = psensor_ssss_data_block_prev->dbHeader.dataElementSize;
    int ret;
    int batch_size;

    p_dest += sizeof(int16_t)*sensor_ssss_samples_collected*dataElementSize;
    ...
    int16_t adc_data0 = qorc_ssi_adc.getSingleEnded(0);
    int16_t adc_data1 = qorc_ssi_adc.getSingleEnded(1);
    //int16_t adc_data2 = qorc_ssi_adc.getSingleEnded(2);
    //int16_t adc_data3 = qorc_ssi_adc.getSingleEnded(3);
    // *p_dest = adc_data0;
    // p_dest += 1;
    ...
    *p_adc_data++ = adc_data0;
    *p_adc_data++ = adc_data1;
    //*p_adc_data++ = adc_data2;
    //*p_adc_data++ = adc_data3;
